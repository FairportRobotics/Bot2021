...
public class FieldPosition {
    ...
    private static TalonSwerveEnclosure[] talonEnclosures = { SwerveDrive.swerveEnclosureFR,
            SwerveDrive.swerveEnclosureBR, SwerveDrive.swerveEnclosureBL, SwerveDrive.swerveEnclosureFL };
    ...
    private static WheelState getWheelState() {
        return new WheelState(getVectFromEnclosure(talonEnclosures[0]), getVectFromEnclosure(talonEnclosures[1]),
                getVectFromEnclosure(talonEnclosures[2]), getVectFromEnclosure(talonEnclosures[3]));
    }

    // returns speed vector of talon
    private static Vector2d getVectFromEnclosure(TalonSwerveEnclosure enclosure) {
        return createVect(angleRadians(enclosure) - Math.toRadians(Robot.gyroSubsystem.getHeading()),
                enclosure.getDriveTalon().getSelectedSensorVelocity());
    }
