...
public class Steer {
    ...
    public void periodic() {
        if (controlMode == ControlMode.AUTO) {
            if (botPath != null) {
                int ind = (int) ((System.currentTimeMillis() - timeInit) * ((double) Points.CURVES_PER_SECOND)
                        * ((double) Points.POINTS_PER_CURVE) / 1000);
        ...
        if (controlMode == ControlMode.USER) {
            ...
            pos = new Vector2d(pos.x * ratio + FieldPosition.getBotXPosition(),
                    pos.y * ratio + FieldPosition.getBotXPosition());
        }
