...
public class SwerveDriveCommand extends Command {
    ...
    protected void execute() {
        ...
        double rotx = rot;
        rot = 0;
        double roty = Robot.oi.rightJoystick.getY();

        if (rotx * rotx + roty * roty > .01) {
            double ang = Math.atan2(roty, rotx);
            prevRot = -MotionProfiling.angPid(ang); // its negative!!!
        ...
        Robot.swerveDriveSubsystem.move(fwd * ratio + profilingPowerX, str * ratio + profilingPowerY,
                prevRot + profilingPowerA, angleDeg);
